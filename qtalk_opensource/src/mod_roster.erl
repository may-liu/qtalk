	  ...
	  RItems = lists:flatmap(fun (I) ->
					 case raw_to_record(LServer, I) of
					   ...
					   R ->
					       ...
					       {UName , _ , _} = R#roster.jid,
					       [R#roster{groups = Groups,name = UName}]
					 end
				 end,
				 Items),
	  RItems;
...
import(_, _, _) ->
    pass.
get_value_by_name(Names,SRes,Name) ->
	Size = length(Names),
	NumList = lists:seq(1,Size),		
	lists:flatmap(fun(Numth) ->
					Rowname = lists:nth(Numth,Names),
					case Rowname == Name of 
					true ->
						lists:map(fun(Orowval) ->
								 lists:nth(Numth,Orowval) end,SRes);
					false ->
						[]	
					end
					end,NumList).
