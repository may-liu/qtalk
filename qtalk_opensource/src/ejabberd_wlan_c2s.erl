...
-define(SUPERVISOR_START, ?GEN_FSM:start(ejabberd_wlan_c2s, [SockData, Opts],
					 fsm_limit_opts(Opts) ++ ?FSMOPTS)).
-else.
-define(SUPERVISOR_START, supervisor:start_child(ejabberd_wlan_c2s_sup,
						 [SockData, Opts])).
-endif.
...
start_link(SockData, Opts) ->
    ?GEN_FSM:start_link(ejabberd_wlan_c2s, [SockData, Opts],
			fsm_limit_opts(Opts) ++ ?FSMOPTS).
...
wait_for_feature_request({xmlstreamelement, El},
    ...
    case {xml:get_attr_s(<<"xmlns">>, Attrs), Name} of
	  ...
	  case cyrsasl:server_start(StateData#state.sasl_state,
		...
		MacKey = proplists:get_value(mac_key, Props, <<"0">>),
	
		?INFO_MSG("(~w) Accepted authentication MacKey ~p for ~s "
			  "by ~p",
			  [StateData#state.socket, MacKey,U, AuthModule]),
		send_element(StateData,
			    #xmlel{name = <<"success">>,
				    attrs = [{<<"xmlns">>, ?NS_SASL}],
				    children = []}),
...
send_text(StateData, Text) ->
	...
	true ->
		?INFO_MSG("[Send XML on stream] ~s", [Text]);
	_ ->
...
send_trailer(StateData) ->
	...
	case str:str(R,<<"Android">>) of 
	0 ->
    	send_text(StateData, ?STREAM_TRAILER);
	_ ->
		?INFO_MSG("No send STREAM_TRAILER ~p/~p@~p ~n",[StateData#state.user,StateData#state.server,StateData#state.resource]),
		ok
	end.
