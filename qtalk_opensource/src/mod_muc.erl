...
-record(muc_node, {name_host = {<<"">>, <<"">>} :: {binary(), binary()} |
                                                   {'_', binary()},
                   node}).
...
init([Host, Opts]) ->
    ...
    case gen_mod:db_type(Opts) of
        mnesia ->
            ...
            mnesia:create_table(muc_node,
                                [{type,set},{disc_copies, [node()]},
                                 {attributes,
                                  record_info(fields, muc_node)}]),
            update_tables(MyHost),
            mnesia:add_table_index(muc_node, name_host),
...
iq_get_unique(From) ->
    {xmlcdata,
		p1_sha:sha(term_to_binary([From, os:timestamp(),
			     randoms:get_string()]))}.
