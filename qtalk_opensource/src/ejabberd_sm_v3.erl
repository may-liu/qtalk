    ...
    case LResource of
	  ...
	  case mnesia:dirty_index_read(session, USR, #session.usr)
		...
		NewPacket  =
			case Name of
		   	<<"message">> ->
				make_new_packet(From,To,Packet,Name,Attrs,Els,Session,LUser,LServer,LResource,true);	
			<<"iq">> ->
...
route_message(From, To, Packet) ->
	...
	_ ->
		...
		#xmlel{name = Name, attrs = Attrs, children = Els} = Packet,
		NewPacket = make_new_packet(From,To,Packet,Name,Attrs,Els,<<"">>,LUser,LServer,<<"">>,false),
...
insert_chat_msg(Server,From, To,From_host,To_host, Msg,_Body,ID,InsertTime) ->
    ...
    LUser ->
	         ...
	         LFrom_host = jlib:nameprep(From_host),
	         LTo_host = jlib:nameprep(To_host),
...
insert_user_chat_msg(From,To,Packet,Name,Attrs,Els,Session,Mbody,LUser,LServer,LResource,Flag) ->
	...
	true  ->
		case Flag of
		true ->
			catch case Session#session.show == <<"away">> andalso
			    (str:str(LResource,<<"PC_Client">>) =/= 0 orelse str:str(LResource,<<"mac">>) =/= 0)  andalso 
...
send_max_priority_msg(LUser,LServer,Priority,From,To,NewPacket,PrioRes) ->
	...
	case judge_away_flag(AwayRes,false) of
	false ->
		ok;
	_ ->
		...
		true ->
		case (not mod_muc_room:is_invitation(Els)) andalso xml:get_tag_attr_s(<<"msgType">>,xml:get_subtag(NewPacket,<<"body">>)) =/= <<"1024">> of 
...
make_new_packet(From,To,Packet,Name,Attrs,Els,Session,LUser,LServer,LResource,Flag) ->
	...
	true ->
		...
		true ->
			case Reply =/= <<"true">> andalso Reply1 =/= <<"true">> of
			true ->
...
consult_message(From,To,Packet) ->
    {ThirdDirection, CN, UsrType} = case xml:get_tag_attr_s(<<"channelid">>, Packet) of
    <<"">> ->
...
virtual_message(<<"chat">>,LServer,From,To,Packet) ->
    case xml:get_tag_attr_s(<<"virtual_msg">>, Packet) of 
    <<"">> ->
            ejabberd_hooks:run(offline_message_hook, LServer,
					    	   	[From, To, Packet]);
    VirtualMsg ->
        {ok, {obj, VirtualJson}, []} = rfc4627:decode(VirtualMsg),        
        VUser = proplists:get_value("virtual_user", VirtualJson, <<"">>),
        handle_virtual_msg(VUser,LServer,From,To,Packet)
    end;
virtual_message(<<"normal">>,LServer,From,To,Packet) ->
    ejabberd_hooks:run(offline_message_hook, LServer,
					    	   	[From, To, Packet]);
virtual_message(_,_,_,_,_) ->
    ok.

handle_virtual_msg(<<"from">>,LServer,From,To,Packet) ->
    ejabberd_hooks:run(offline_message_hook, LServer,
        [From, To, Packet]);
handle_virtual_msg(<<"to">>,LServer,From,To,Packet) ->
    do_handle_virtual_msg(LServer,From,To,Packet);
handle_virtual_msg(_,_,_,_,_) ->
    ok.

do_handle_virtual_msg(LServer,From,To,Packet) ->
    Session = get_virtual_session(LServer,From,To),
    ok.


get_virtual_session(LServer,From,To) -> 
    case catch ets:lookup(virtual_session,str:concat(To#jid.luser,From#jid.luser)) of
   %% [{_,Session}] when is_record(Session,virtual_session) ->
    [{_,Session}] ->
        Session;
    _ ->
        'undefined'
...
send_virtual_msg_to_realjid(Vsession,LServer,From,To,Packet) ->
    case mnesia:dirty_index_read(session,Vsession,#session.us) of
    [] ->
        Session = assign_new_session(Vsession,LServer,From,Packet);
    L when is_list(L)  ->
        lists:foreach(fun(S) ->
            {_,Pid} = S#session.sid,
            To = jlib:make_jid(S#session.usr),
            Pid ! {route, From, To, Packet} end,L);
    _ ->
        ok
    end;
send_virtual_msg_to_realjid('undefined',LServer,From,To,Packet) ->
    Session = assign_new_session(LServer,From,To,Packet).
    
assign_new_session(Server,From,To,Packet) ->   
    case catch ets:lookup(virtual_list,From#jid.luser) of
    [{_,L}] when is_list(L) ->
        case hash_get_realto_jid(L,L) of
        'undefined' ->
            'undefined';
        JID ->
            make_new_session(Server,From,To,JID,Packet)
        end;
    _ ->
        ok
    end.
...
hash_get_realto_jid([],UserList) ->
    'undefined';
hash_get_realto_jid(L,UserList) when is_list(L)->
    {User,Server} = lists:nth(erlang:phash(os:timestamp(), length(L)), L),
    case catch ejabberd_sm:get_user_rescoure(User,Server) of
    [] ->
        hash_get_realto_jid(L -- [{User,Server}],UserList);
    _ ->
        if (L =:= UserList) ->
            ok;
        true ->
...
get_session_timestmap(User,Server) ->
    USR = {User, Server},
    case catch mnesia:dirty_index_read(session, USR, #session.us) of
    [] ->
        [];
    L when is_list(L) ->
        lists:map(fun(S) ->
            {{T1,T2,_},_} = S#session.sid,
            {_,_,R} = S#session.usr,
            {T1*1000000+T2,R} end,L);
    _ ->
        []
    end.
