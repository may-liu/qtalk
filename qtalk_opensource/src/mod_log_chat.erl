...
start(Host, Opts) ->
    case gen_mod:get_opt(host_config, Opts, fun(A) -> A end, []) of
	[] ->
	    start_vh(Host, Opts);
	HostConfig ->
...
start_vhs(Host, [{_VHost, _Opts}| Tail]) ->
    start_vhs(Host, Tail).
start_vh(Host, Opts) ->
    Path = gen_mod:get_opt(path, Opts, fun(A) -> A end, ?DEFAULT_PATH),
    Format = gen_mod:get_opt(format, Opts, fun(A) -> A end, ?DEFAULT_FORMAT),
...
init(Config)->
    ?INFO_MSG("Starting ~p with config ~p", [?MODULE, Config]),
    loop(Config).
...
log_packet(From, To, Packet = {xmlel, <<"message">>, Attrs, _Els}, Host) ->
    case xml:get_attr_s(<<"type">>, Attrs) of
	<<"groupchat">> -> %% mod_muc_log already does it
%%	    ?INFO_MSG("dropping groupchat: ~p", [Packet]),
%	    ?INFO_MSG("dropping groupchat: ~p", [xml:element_to_string(Packet)]),
	    ok;
	<<"error">> -> %% we don't log errors
	    ?INFO_MSG("dropping error: ~p", [xml:element_to_string(Packet)]),
	    ok;
	_ ->
...
write_packet(From, To, Packet, Host) ->
    ...
    {Subjectb, Bodyb} = {case xml:get_path_s(Packet, [{elem, <<"subject">>}, cdata]) of
			   false ->
			       "";
			   Text ->
			       escape(Format, Text)
	...
	{Subject, Body} = {binary_to_list(Subjectb), binary_to_list(Bodyb)},
...
%            ?INFO_MSG("not logging empty message from ~s",[jlib:jid_to_string(From)]),
	    ...
	    FromJid = binary_to_list(From#jid.luser)++"@"++binary_to_list(From#jid.lserver),
	    ToJid = binary_to_list(To#jid.luser)++"@"++binary_to_list(To#jid.lserver),
...
%	    ?INFO_MSG("FilenameTemplate ~p",[FilenameTemplate]),
	    Filename = io_lib:format(FilenameTemplate, [Y, M, D]),
%	    ?INFO_MSG("logging message from ~s into ~s",[jlib:jid_to_string(From), Filename]),
...
%	    ?INFO_MSG("MessageTemplate ~s",[template(Format, MessageType)]),
		...
		?INFO_MSG("from ~p to ~p ~p", [FromJid, ToJid, MessageText]),
	    file:close(File)
    end.
...
open_logfile(Filename) ->
	...
	{ok, File} ->
	    File;
	{error, Reason} ->
	    ?ERROR_MSG("Cannot write into file ~s: ~p", [Filename, Reason])
    end.
