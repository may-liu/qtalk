...
get_echo(<<"GET">>,Req) ->
     ...
     Res =     set_whitelist(Username, Flag),
...
do_set_whitelist(Username, Flag) ->
    ...
    F = fun() ->
                odbc_queries:update_whitelist(Username, Flag)
        end,            
    case odbc_queries:sql_transaction(LServer, F) of
        {atomic, _} ->
            whereis(mod_update) ! white_blacklist,
            http_utils:gen_result(true, 0, <<"set whitelist success">>);
        {aborted, Reason} ->
            ?DEBUG("set white list fail, reason is ~p~n", [Reason]),
...
delete_whitelist(Username) ->
    Servers = ejabberd_config:get_myhosts(),
    LServer = lists:nth(1,Servers),
	case odbc_queries:delete_whitelist(LServer, Username) of
	{updated, _} ->
		whereis(mod_update) ! white_blacklist,
	...
	{aborted, Reason} ->
        ...
        http_utils:gen_result(false, http_utils:to_integer(proplists:get_value(code, Reason)), proplists:get_value(message, Reason));
	_ ->
        http_utils:gen_result(false, 2, <<"Unknown error">>)
	end.
