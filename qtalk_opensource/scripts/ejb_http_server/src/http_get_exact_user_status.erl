...
-module(http_get_exact_user_status).
...
handle(Req, State) ->
    catch ejb_monitor:monitor_count(<<"http_get_exact_user_status">>,1),
...
%%	case http_utils:verify_user_key(Req) of
	...
	_ ->
		cowboy_req:reply(200, [ {<<"content-type">>, <<"text/json; charset=utf-8">>}], 
				http_utils:gen_result(false, <<"1">>, <<"user and key not match">>,[]), Req)
	end;
